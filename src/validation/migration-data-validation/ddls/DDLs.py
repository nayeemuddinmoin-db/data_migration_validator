...
spark.sql(f'''
CREATE OR REPLACE TABLE {UPDATE_TSTMP_TIMELINE}
(
  run_timestamp TIMESTAMP,
  type STRING,
  date_bucket STRING,
...
spark.sql(f'''
CREATE OR REPLACE TABLE {PRIMARY_KEY_VALIDATION} (
  run_timestamp timestamp,
  table_name STRING,
...
spark.sql(f'''
CREATE OR REPLACE TABLE {DATA_TYPE_COMPATIBILITY_MATRIX} (
  warehouse STRING,
  data_type STRING,
...
spark.read.option("header","true")\
    ...
    .csv("file://"+os.path.abspath("../conf/data_type_compatibility_matrix.csv"))\
    .write.mode("overwrite").saveAsTable(f'''{DATA_TYPE_COMPATIBILITY_MATRIX}''')

# COMMAND ----------

# import os
# loc = "file:"+os.path.abspath("../conf/data_type_compatibility_matrix.csv")
# spark.sql(f'''
# DROP TABLE IF EXISTS {VALIDATION_METRICS_DB}.db_data_type_compatibility_matrix
# ''')
# spark.sql(f'''
# CREATE TABLE {VALIDATION_METRICS_DB}.db_data_type_compatibility_matrix (
#   warehouse STRING,
#   data_type STRING,
#   has_precision BOOLEAN,
#   databricks_compatibility_regex STRING,
